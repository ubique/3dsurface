...
TripleBuffer::TripleBuffer(std::array<void *, 3> buffers) : buffers_(buffers) {
  flags_ =
      0 | (1 << spare_buffer_index_shift) | (2 << producer_buffer_index_shift);
}
...
void TripleBuffer::switch_producer_buffer() {
  ...
  const unsigned spare_index =
      (flags & spare_buffer_index_mask) >> spare_buffer_index_shift;
  const unsigned producer_index =
      (flags & producer_buffer_index_mask) >> producer_buffer_index_shift;
